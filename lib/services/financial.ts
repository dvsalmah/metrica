...
export function generateEmptyCashflows(periodType: 'monthly' | 'yearly' = 'monthly', length: number = 3): MonthlyCashflow[] {
  const count = periodType === 'monthly' ? length * 12 : length;
 ...
 */
export function buildSampleCashflows(periodType: 'monthly' | 'yearly' = 'monthly', length: number = 3): MonthlyCashflow[] {
  const count = periodType === 'monthly' ? length * 12 : length;
  return Array.from({ length: count }, (_, i) => {
    let netCashflow = 0;
    if (periodType === 'monthly') {
      ...
      netCashflow = base[i % base.length] || 4000000;
    } else {
      const base = [15000000, 30000000, 40000000, 45000000, 50000000];
      netCashflow = base[i % base.length] || 50000000;
    }
    return { month: i + 1, netCashflow };
  });
}
